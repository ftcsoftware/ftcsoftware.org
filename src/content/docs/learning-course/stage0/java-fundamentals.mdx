...
Some example data types that are commonly used in FTC programming are:
...
In FTC, variables can be used to hold information about the robot and its different mechanisms.
...
<Aside type="note">
In FTC, we tend to avoid print statements and instead have other methods of debugging code, which we will talk about in a later section.
